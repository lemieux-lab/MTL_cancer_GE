...
"nfolds" => 5,  "nepochs" => 10_000, "mb_size" => 50,"wd" => 1e-1, "dim_redux" => 16, "enc_nb_hl" => 2, 
"enc_hl1_size"=>32,"enc_hl2_size"=>32,
"nb_clinf" => 0,"cph_lr" => 1e-5, "cph_nb_hl" => 2, "cph_hl_size" => 32)
### NEW STUFF
### determine reduction layer DONE 
### add clinical data to CPH DNN training 

dump_cb_brca = dump_model_cb(1000, labs_appdf(brca_prediction.subgroups), export_type = "pdf")
model = build(brca_cphdnn_params)
validate_2!(brca_cphdnn_params, brca_prediction, dump_cb_brca)

function validate_2!(brca_cphdnn_params, brca_prediction, dump_cb_brca;device = gpu)
    folds = split_train_test(Matrix(brca_prediction.data), Matrix(clinf[:,2:end]), brca_prediction.survt, brca_prediction.surve, brca_prediction.rows;nfolds =5)
    #device()
    #device!()
    ...
    for fold in folds
        ...
        for iter in 1:nepochs#ProgressBar(1:nepochs)
            ...
            OUTS = vec(model.cphdnn(model.encoder(gpu(train_x))))
            ...
            cind_test = round(concordance_index(test_y_t, test_y_e,  vec(model.cphdnn(model.encoder(test_x))) )[1], digits =3)
        ...
        OUTS = cpu(vec(model.cphdnn(model.encoder(test_x))))
...
function validate_1!(brca_cphdnn_params, brca_prediction, dump_cb_brca;device = gpu)
    folds = split_train_test(Matrix(brca_prediction.data), Matrix(clinf[:,2:end]), brca_prediction.survt, brca_prediction.surve, brca_prediction.rows;nfolds =5)
    #device()
    #device!()
    mkdir("RES/$(brca_cphdnn_params["session_id"])/$(brca_cphdnn_params["modelid"])")
    ...
    for fold in folds
        ...
        for iter in 1:nepochs#ProgressBar(1:nepochs)
            ...
            OUTS = vec(model.cphdnn(vcat(model.encoder(gpu(train_x)), gpu(train_x_c))))
            cph_loss = model.lossf(model,train_x, train_x_c, train_y_e, NE_frac_tr, brca_cphdnn_params["wd"])
            cind_tr, cdnt_tr, ddnt_tr  = concordance_index(train_y_t, train_y_e, OUTS)
            brca_cphdnn_params["tr_acc"] = cind_tr
            #push!(learning_curve, (ae_loss, ae_cor, cph_loss, cind_tr))
