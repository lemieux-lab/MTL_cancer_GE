...
"nsamples" => length(brca_prediction.samples) , "insize" => insize, "ngenes" => length(brca_prediction.genes),  
"nfolds" => 5,  "nepochs" => 3_000, "mb_size" => 50,"wd" => 1e-1,
"enc_hl1_size" => 128, "enc_hl2_size" => 128, "dim_redux"=> 128, 
...
train_x_ = train_x_c# vcat(train_x, train_x_c)
...
test_x_ = test_x_c#vcat(test_x, test_x_c)
    
NE_frac_tst = sum(test_y_e .== 1) != 0 ? 1 / sum(test_y_e .== 1) : 0
model = build(brca_cphdnn_params)
vec(model.cphdnn(vcat(model.encoder(train_x), train_x_c)))
...
learning_curve = []
params_dict = brca_cphdnn_params
#params_dict["nsamples_test"]
opt = Flux.ADAM(params_dict["cph_lr"])
#model = dnn(gpu(Chain(Dense(params_dict["nb_clinf"], 1, sigmoid))), opt,cox_l2)#build(brca_cphdnn_params)
for iter in 1:30_000#ProgressBar(1:nepochs)
    ...
    OUTS_tr = vec(model.model(train_x_))
    cph_loss = model.lossf(model,train_x_, train_y_e, NE_frac_tr, brca_cphdnn_params["wd"])
    cind_tr, cdnt_tr, ddnt_tr  = concordance_index(train_y_t, train_y_e, OUTS_tr)
    ...
    OUTS_tst = vec(model.model(test_x_))
    cind_test = round(concordance_index(test_y_t, test_y_e,  OUTS_tst)[1], digits =3)
    push!(learning_curve, ( cph_loss, cind_tr, cph_loss_test, cind_test))
    println("FOLD $(fold["foldn"]) - $iter\t TRAIN cph-loss-avg: $(round(cph_loss / params_dict["nsamples_train"],digits =5)) \t cph-cind: $(round(cind_tr ,digits =3))\t TEST cph-loss-avg: $(round(cph_loss_test / params_dict["nsamples_test"],digits =6)) \t cph-cind: $(round(cind_test,digits =3))")
...
hist!(fig[1,1], OUTS )
fig
concordance_index(cpu(train_y_t), cpu(train_y_e), OUTS )
fig = Figure();
ax = Axis(fig[1,1])
...
function validate_enccphdnn!(brca_cphdnn_params, brca_prediction, dump_cb_brca, clinf;device = gpu)
    folds = split_train_test(Matrix(brca_prediction.data), Matrix(clinf[:,2:end]), brca_prediction.survt, brca_prediction.surve, brca_prediction.samples;nfolds =5)
    #device()
    #device!()
    ...
    for fold in folds
        opt = Flux.ADAM(brca_cphdnn_params["cph_lr"])
        model = build(brca_cphdnn_params)
        #function train!(model::mtl_cph_AE, fold, dump_cb, params)
            ## mtliative Auto-Encoder + Classifier NN model training function 
            ## Vanilla Auto-Encoder training function 
        ...
        for iter in 1:nepochs#ProgressBar(1:nepochs)
            ...
            cind_test = round(concordance_index(test_y_t, test_y_e, OUTS_tst)[1], digits =3)
            push!(learning_curve, ( cph_loss, cind_tr, cph_loss_test, cind_test))
            println("FOLD $(fold["foldn"]) - $iter\t TRAIN cph-loss-avg: $(round(cph_loss / params_dict["nsamples_train"],digits =6)) \t cph-cind: $(round(cind_tr ,digits =3))\t TEST cph-loss-avg: $(round(cph_loss_test / params_dict["nsamples_test"],digits =6)) \t cph-cind: $(round(cind_test,digits =3))")
            #dump_cb_brca(model, EncCPHDNN_LearningCurve(learning_curve), brca_cphdnn_params, iter, fold)
...
function validate_cph!(brca_cphdnn_params, brca_prediction, dump_cb_brca, clinf;device = gpu)
    folds = split_train_test(Matrix(brca_prediction.data), Matrix(clinf[:,2:end]), brca_prediction.survt, brca_prediction.surve, brca_prediction.samples;nfolds =5)
    #device()
    #device!()
    mkdir("RES/$(brca_cphdnn_params["session_id"])/$(brca_cphdnn_params["modelid"])")
    ...
    for fold in folds
        opt = Flux.ADAM(brca_cphdnn_params["cph_lr"])
        #model = build(brca_cphdnn_params)
        model = dnn(gpu(Chain(Dense(brca_cphdnn_params["nb_clinf"], 1, sigmoid))), opt,cox_l2)#build(brca_cphdnn_params)
        #function train!(model::mtl_cph_AE, fold, dump_cb, params)
            ## mtliative Auto-Encoder + Classifier NN model training function 
            ## Vanilla Auto-Encoder training function 
        ...
        for iter in 1:nepochs#ProgressBar(1:nepochs)
            ...
            push!(learning_curve, ( cph_loss, cind_tr, cph_loss_test, cind_test))
            println("FOLD $(fold["foldn"]) - $iter\t TRAIN cph-loss-avg: $(round(cph_loss / params_dict["nsamples_train"],digits =3)) \t cph-cind: $(round(cind_tr ,digits =3))\t TEST cph-loss-avg: $(round(cph_loss_test / params_dict["nsamples_test"],digits =3)) \t cph-cind: $(round(cind_test,digits =3))")
            #dump_cb_brca(model, EncCPHDNN_LearningCurve(learning_curve), brca_cphdnn_params, iter, fold)
